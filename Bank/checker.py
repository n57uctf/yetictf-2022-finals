...
class CheckMachine:
    ...
    def register(self, name, passwd):
        ...
        resp = sess.post(f'{self.url}/registration', data={"login": name, "password": passwd})
    ...
    def login(self, name, passwd):
        ...
        resp = sess.post(f'{self.url}/login', data={'name': name, 'passwd': passwd})
        check_response(resp, 'Could not login')

        return sess

    def profile(self, sess):
        resp = sess.get(f'{self.url}/account')
        check_response(resp, "Can't get account page")
